...
const WebSeriesDetail = (props) => {
  ...
  //     if (movieDetails[0]?.convertUpdateType?.thumbnail == 1) {
  //       const fetchData = async () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (movieDetails[0]?.convertUpdateType?.thumbnail === 1 ||
        movieDetails[0]?.convertUpdateType?.image === 1 ||
        movieDetails[0]?.convertUpdateType?.link === 1)
    ) {
      ...
      fetchThumbnail();
      const interval = setInterval(fetchThumbnail, 1000 * 60);
      return () => clearInterval(interval);
    } else {
      
      setShowImage({
        thumbnailImage: movieDetails?.length > 0 ? movieDetails[0]?.thumbnail : "",
        image: movieDetails?.length > 0 ? movieDetails[0]?.image : "",
        trailerVideo: movieDetails?.length > 0 ? movieDetails[0]?.trailer?.videoUrl : "",
      });
    }
  }, [movieDetails]);
  ...
  useEffect(() => {
    if (movieDetails?.length > 0) {
      ...
      fetchRoleData();
      const interval = setInterval(fetchRoleData, 1000 * 60);
      return () => clearInterval(interval);
    }
  }, [movieDetails]);
    ...
    if (movieDetails?.length > 0) {
      const fetchEpisodeData = async () => {
        const episodeUrls = await Promise.all(
          movieDetails[0]?.episode?.length > 0 &&
          movieDetails[0]?.episode?.map(async (item) => {
            if (item?.convertUpdateType?.image === 1) {
              const fileNameWithExtension = item?.image.split("/").pop();
              const { imageURL } = await covertURl(
                "episodeImage/" + fileNameWithExtension
              );
      ...
      fetchEpisodeData();
      const interval = setInterval(fetchEpisodeData, 1000 * 60);
      return () => clearInterval(interval);
    }
  }, [movieDetails]);
